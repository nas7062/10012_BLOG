...
setup("로그인 세션 저장", async ({ page, context }) => {
  if (!email || !password) {
    throw new Error(
      "TEST_USER_EMAIL 또는 TEST_USER_PASSWORD가 설정되지 않았습니다."
    );
  }
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      ...
      if (
        e instanceof Error &&
        (e.message.includes("closed") || e.message.includes("Target closed"))
      ) {
  ...
  if (!hasSessionCookie) {
    ...
    try {
      await page.screenshot({
        path: "test-results/login-no-cookie.png",
        fullPage: true,
      });
    } catch {}
