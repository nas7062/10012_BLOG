...
import { IPost, IRepple, IUser } from "../../../types";
...
export function usePostDetailData(postId: number, initialData?: InitialData) {
  const { data: session } = useSession();
  const email = session?.user?.email ?? "";

  // initialData가 있으면 서버 데이터를 initialData로 사용
  const { data: writeUser, isLoading: isAuthorLoading } = usePostAuthor(postId);
